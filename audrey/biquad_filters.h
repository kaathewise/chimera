...
#ifndef AUDREY_BIQUAD_FILTERS_H_
#define AUDREY_BIQUAD_FILTERS_H_
 ...
 * processing. NOTE: There are templated filter definitions at the bottom of
...
class BiquadSection {
 public:
  ...
  inline float Process(const float in, const int channel) {
    ...
    // TODO(susachev): arm accelerated version
...
class BiquadCascade {
 ...
 public:
  ...
  /// Sets Q values in each section for a truly "flat" (-3dB cutoff point)
  /// response NOTE: per limitations of this class this only works for
  /// even-order filters
 ...
 private:
  ...
  inline void updateCoefficients() {
    for (size_t i = 0; i < NumSections; i++) {
      biquads_[i].SetCoefficients(BiquadSection::CalculateCoefficients(
          FilterType, sample_rate_, cutoff_hz_, q_[i]));
    }
  }
};
