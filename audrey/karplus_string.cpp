...
float KarplusString::Process(const float in) { return ProcessInternal(in); }
...
float KarplusString::ProcessInternal(const float in) {
  ...
  // interpolator. We don't care because it's a corner case (frequency_
  // < 11.7Hz)
  ...
  // float damping_f
  // = fmin(frequency_ * powf(2.f, damping_cutoff * daisysp::kOneTwelfth),
  // 0.499f);
  ...
  // float ratio                = powf(2.f, damping_cutoff *
  // daisysp::kOneTwelfth); float damping_compensation = 1.f - 2.f * atanf(1.f /
  // ratio) / (TWOPI_F);
